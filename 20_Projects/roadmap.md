...
最終更新: 2026-04-02（orchestrate: #15[13]・#10[11]・#6[14] Top3実行。#15: 通知待機中週次アクション・落ちる理由チェックリスト・当日移動準備追加。#10: タイムリミット+6日エスカレーション更新。#6: roadmap整合（GitHub closed確認）+ 様式6ブラッシュアップ追加。GitHub close: 権限制限のためCLI不可（手動要）。）
...
- [~] #15 面接対策 `priority:high` → 想定問答集30問・HPP世界唯一スクリプト・琉大固有情報・言語切り替えドリル・面接後アクション完備。2026-04-01: 面接通知受領後即時起動プロトコル追加。実練習はユーザー対応。GitHub close: 権限制限のためCLI不可（手動要）
- [~] #8 様式8（診療に関する抱負）ブラッシュアップ `priority:high` → 構成ガイド・沖縄医療課題・DX・経営視点フレームワーク作成済み（数値記入はユーザー対応）
- [ ] #9 様式5（主要論文要旨）確認
- [~] #10 数値系書類（様式2・3・4・9）最終確認 → 様式9: 推計即記入ワークシート追加済み（2026-04-01）。実データ入力はユーザー対応。様式2記入日空欄は1分で解消可。
- [~] #10 推薦書（様式1）推薦者確定・依頼 → 推薦依頼状ドラフト・業績サマリー・候補者評価マトリクス・進捗トラッカー・フォローアップテンプレート3種（承諾後/リマインダー/受領確認）・運用チェックポイント追加済み（2026-03-18）。推薦者2名確定・推薦書受領済み。🚨 推薦書受領確認アクションシート作成（2026-03-25）: 推薦書①日付「令和8年8月13日」要確認・②署名・日付未記入→修正依頼テンプレート追加。**2026-03-28更新: 🆘タイムリミット超過1日。推薦書②催促が実質最終期限。緊急エスカレーション追加（推薦書_受領確認アクションシート.md更新）**。**2026-03-31更新: タイムリミット超過+4日。R4フロー（同僚経由・推薦者変更手順）追加。今日の受領確認 or R4発動はユーザー判断。修正受領・格納はユーザー対応**。**2026-04-02更新: タイムリミット超過+6日。推薦者変更意思決定ガイド追加。今日中に「変更 or 受領」を決断する分岐点。ユーザー判断・対応必須**
