...
### 7. 生成マーカー（すかし）禁止
**出力先を問わず適用**（brain内・brain外の作業フォルダ・論文プロジェクト等すべて）。

- 生成した文章に、生成元・実行者・AIモデル名を示すマーカーを**一切書かない**
- 禁止する場所: 本文 / 末尾フッター / 冒頭メタ行 / frontmatter / HTML・LaTeXコメント / ファイルのメタデータ
- 禁止する表記例:
  - `*Generated by /orchestrate — YYYY-MM-DD*`
  - `_生成: Claude Code (claude-xxx) / orchestrate_`
  - `実行者: Claude ...` / `自動生成: ...` / `🤖` などの絵文字マーカー
  - `Generated with Claude Code` / `Co-Authored-By: Claude` / `% Written by AI`
- コマンド名の記録が必要な場合のみ `実行コマンド: \`/orchestrate\`` の形式で書く（AI・モデル名は書かない）
- レポート本文の見出し・表・箇条書きのみで完結させる
- 例外: GitHubのIssue/PRコメントに自動付与されるフッター（Claude Code側の仕様で抑制不可）
